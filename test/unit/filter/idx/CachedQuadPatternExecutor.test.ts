...
describe('A CachedQuadPatternExecutor', (): void => {
  ...
  it('supports the merged representations generated by a SelectorHandlerCachedStore.', async(): Promise<void> => {
    const merged = createStoreRepresentation(
      new Store(quads),
      { path: 'http://example.com/derived' },
      new Date(1988, 2, 9),
    );
    // Using the real source here, as the point is that the full chain works on a merged representation
    const realParser = new CachedQuadPatternExecutor(new BaseQuadPatternExecutor());
    const result = await realParser.handle({ filter, representation: merged });
